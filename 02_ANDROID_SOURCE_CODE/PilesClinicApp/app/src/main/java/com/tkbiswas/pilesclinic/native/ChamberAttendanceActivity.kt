...
class ChamberAttendanceActivity : AppCompatActivity() {
    ...
    // 🔴🔒 V1618 (২০.০৯.২০২৬, TK-নির্দেশ) — "Mark Expected"-এ আগে শুধু মোবাইল
    // দিয়েই খোঁজা যেত; TK: "নাম দিয়েও যাতে করা যায় তার ব্যবস্থা করুন"।
    // নাম-খোঁজা মানে শেষে মোবাইলটাই বসানো (Save-এর জন্য মোবাইল বাধ্যতামূলক),
    // তাই এটা মোবাইল-খোঁজার হুবহু উল্টো দিক — একই প্রমাণিত পথ
    // (`ChamberAttendanceRepository.searchPatients`, TK-অনুমোদিত ১৬.০৭.২০২৬
    // থেকেই লাইভ) ব্যবহার করে, শুধু রেজিস্টার্ড রোগীর পাশাপাশি এনকোয়ারিও
    // খোঁজা হয় (ডায়ালগের নিজের কথা: "Works for an Enquiry, a Patient, or
    // someone with no record yet")।
    private data class ChamberNameMatch(val name: String, val mobile: String, val branch: String)

    private suspend fun searchByNameForExpected(query: String, branchFilter: String?): List<ChamberNameMatch> =
        withContext(Dispatchers.IO) {
            val out = LinkedHashMap<String, ChamberNameMatch>()
            try {
                for (p in ChamberAttendanceRepository.searchPatients(query, branchFilter)) {
                    val d = p.mobile.filter { it.isDigit() }.takeLast(10)
                    if (d.length == 10) out[d] = ChamberNameMatch(p.name, d, p.branch)
                }
            } catch (_: Throwable) { }
            try {
                val pattern = java.net.URLEncoder.encode("*$query*", "UTF-8")
                val orFilter = "name.ilike.$pattern"
                val filter = if (!branchFilter.isNullOrBlank() && branchFilter != "All")
                    "branch=eq.${java.net.URLEncoder.encode(branchFilter, "UTF-8")}&$orFilter" else orFilter
                val enq = SupabaseClient.fetchListSlim("enquiries", filter, 20, "name,mobile,branch")
                for (i in 0 until enq.length()) {
                    val row = enq.optJSONObject(i) ?: continue
                    val d = row.s("mobile").filter { it.isDigit() }.takeLast(10)
                    if (d.length == 10 && !out.containsKey(d)) out[d] = ChamberNameMatch(row.s("name"), d, row.s("branch"))
                }
            } catch (_: Throwable) { }
            out.values.toList()
        }

    private suspend fun askWhichByName(query: String, matches: List<ChamberNameMatch>): ChamberNameMatch? =
        kotlinx.coroutines.suspendCancellableCoroutine { cont ->
            val labels = matches.map { (it.name.ifBlank { "UNKNOWN" }) + "\n📞 " + it.mobile }.toTypedArray()
            var done = false
            fun finishWith(v: ChamberNameMatch?) { if (done) return; done = true; if (cont.isActive) cont.resume(v) }
            if (isFinishing || isDestroyed) { finishWith(null); return@suspendCancellableCoroutine }
            AlertDialog.Builder(this)
                .setTitle("🔎 \"$query\" — which one?")
                .setItems(labels) { _, which -> finishWith(matches.getOrNull(which)) }
                .setNegativeButton("Cancel") { _, _ -> finishWith(null) }
                .setOnCancelListener { finishWith(null) }
                .show().also { try { com.tkbiswas.pilesclinic.native.NoAutofill.scrubAnyDialog(it) } catch (_: Throwable) { } }   // 🤫 V774
        }
    ...
    override fun onResume() {
        ...
        autoWatch.startRealtime({ selectedBranch }) {
            if (!::adapter.isInitialized || !autoScreenFocused || !isToday()) return@startRealtime
    ...
    private fun showMarkExpectedDialog() {
        ...
        val nameInput = android.widget.EditText(this).apply {
            hint = "Name (type to search, or auto-fills from mobile)"
            setBackgroundResource(com.tkbiswas.pilesclinic.R.drawable.bg_input_field)
            setPadding(dp(14), dp(12), dp(14), dp(12))
        }
        ...
        // 🔴🔒 V1618 (২০.০৯.২০২৬, TK-নির্দেশ) — মোবাইলের উল্টো দিক: নাম টাইপ
        // করলে মোবাইল খুঁজে বসানো। মোবাইল আগে থেকে (১০ সংখ্যা) ভরা থাকলে এই
        // খোঁজা দাঁড়িয়ে যায় — দুটো ঘর একে অপরকে ওভাররাইট করে না।
        var nameMatchToken = 0
        nameInput.addTextChangedListener(object : android.text.TextWatcher {
            ...
            override fun afterTextChanged(s: android.text.Editable?) {
                ...
                if (mobileInput.text.toString().filter { it.isDigit() }.length == 10) return
                val myToken = ++nameMatchToken
                lifecycleScope.launch {
                    val branchScope = if (user.role == "master") null else user.branch
                    val found = withContext(Dispatchers.IO) {
                        try { searchByNameForExpected(q, branchScope) } catch (_: Throwable) { emptyList() }
                    }
                    if (myToken != nameMatchToken || found.isEmpty()) return@launch
                    if (mobileInput.text.toString().filter { it.isDigit() }.length == 10) return@launch
                    val chosen = if (found.size == 1) found[0] else (askWhichByName(q, found) ?: return@launch)
                    if (myToken != nameMatchToken) return@launch
                    mobileInput.setText(chosen.mobile)
                    if (chosen.name.isNotBlank()) nameInput.setText(chosen.name)
                    if (chosen.branch.isNotBlank()) matchedBranch = chosen.branch
                }
            }
        })
    ...
    private fun showSearchDialog() {
        // 🔴🔒 V1619 (২০.০৯.২০২৬, TK-নির্দেশ, ডেমো-প্রুফ পাশ) — "পপ আপের উচ্চতা
        // আরো অনেক বড় হবে": স্থির 400dp-র বদলে ফোনের আসল উচ্চতার ~৬২% —
        // বড় ফোনে বেশি সারি একসাথে দেখা যায়, ছোট ফোনেও উপচে পড়ে না।
        val screenHeightDp = (resources.displayMetrics.heightPixels / resources.displayMetrics.density).toInt()
        val parts = premiumDialogShellChamber("🔍 Search Patient", (screenHeightDp * 0.62).toInt())
        val d = resources.displayMetrics.density
        ...
        fun runSearch() {
            ...
            lifecycleScope.launch {
                ...
                results.forEach { res ->
                    ...
                    card.addView(row)
                    // 🔴🔒 V1619 (২০.০৯.২০২৬, TK-নির্দেশ, ডেমো-প্রুফ পাশ) — আগে নাম
                    // দুই লাইন পর্যন্ত যেতে পারত (লম্বা নামে কার্ড লম্বা হয়ে যেত),
                    // আর রোগের চিপ ছিল নামের পাশে। TK-অনুমোদিত নতুন সাজ: নাম এক
                    // লাইনে (লম্বা হলে শেষে "…") · তার নিচে মোবাইল · তার নিচে
                    // Patient ID-র পাশে রোগের চিপ।
                    val info = android.widget.LinearLayout(this@ChamberAttendanceActivity).apply {
                        ...
                        if (res.patientId.isNotBlank() || res.disease.isNotBlank()) {
                    ...
                    // 🔴🔒 V1619 — বোতাম আগের চেয়ে ছোট (প্যাডিং ও লেখার মাপ দুটোই কমানো)।
     ...
     */
    private suspend fun resolveBestFollowUpId(digits: String, patient: ChamberAttendanceRow? = null): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val rows = SupabaseClient.findByMobileOrNull(
                "followups", "+91$digits", "id,refId,patientId,name,status,stage,updatedAt,createdAt", 50
            ) ?: return@withContext null
            val all = (0 until rows.length()).map { rows.getJSONObject(it) }
            val pool = if (patient == null) all else {
                val rid = patient.patientRowId.trim()
                val code = patient.patientId.trim()
                // V1653 same-mobile identity precedence:
                // 1) refId (patients row id) is the strongest proof and always wins.
                // 2) Patient-ID code is used only when the row is NOT proven to belong
                //    to another declared patient. This matters for legacy inconsistent
                //    rows where refId points to patient-B but patientId still contains
                //    patient-A's old code.
                val exactByRef = if (rid.isBlank()) emptyList() else all.filter { it.s("refId") == rid }
                val exactByCode = if (code.isBlank()) emptyList() else all.filter {
                    it.s("patientId").equals(code, ignoreCase = true) &&
                        !PatientIdentity.provablyOtherPatient(it, digits, rid, code, patient.name)
                }
                when {
                    exactByRef.isNotEmpty() -> exactByRef
                    exactByCode.isNotEmpty() -> exactByCode
                    PatientModel.isDeclaredSeparateRowId(rid, digits) -> emptyList()
                    else -> all.filterNot {
                        PatientIdentity.provablyOtherPatient(it, digits, rid, code, patient.name)
                    }
                }
            ...
            when {
                ...
                pool.isNotEmpty() -> terminalFollowUpSentinel
                else -> ""
            }
        } catch (_: Throwable) { null }
    }
    ...
    private suspend fun resolveOrHealFollowUpId(row: ChamberAttendanceRow, digits: String): String {
        val resolvedId = resolveBestFollowUpId(digits, row)
        ...
        val newId = row.patientRowId.trim().takeIf { it.isNotBlank() }
            ?.let { "fu_pat_$it" }
            ?: ("fu_" + java.util.UUID.randomUUID().toString().replace("-", ""))
        val healRow = org.json.JSONObject()
            .put("id", newId)
            .put("refId", row.patientRowId)
    ...
    /* 🔴🔴🔒 V1157 (০৭.০৯.২০২৬, TK-রিপোর্ট ছবিসহ: *"উক্ত দিনে চেম্বার বন্ধ করা
       হয়েছিল … ট্রিটমেন্ট প্রগ্রেসের ঘর খালি … নিয়ম অনুসারে খালি থাকলে চেম্বার
       বন্ধ হওয়ার কথাই না … এই একই সমস্যার কথা বেশ কয়েকবার বলেছি"*)।
       ─── আসল কারণ (কোডে মেপে পাওয়া, আন্দাজ নয়) ─────────────────────────────
       V810-এ (২৮.০৮.২০২৬) ঠিক এই দোষটাই ধরা পড়েছিল — বোর্ডের ঘরে অন্য দিনের
       পুরনো লেখা বসে থাকায় পাহারা "লেখা আছে" ধরে বন্ধ করতে দিত। কিন্তু ওই
       সারানোটা বসেছিল **শুধু আজকের চেম্বারে**; নিচের লাইনটা পুরনো দিনের জন্য
       সোজা `false` ফেরত দিত, অর্থাৎ **বিগত দিনের চেম্বারে পাহারাটা আসলে
       বন্ধই ছিল** — যে কোনো পুরনো লেখা থাকলেই বন্ধ হয়ে যেত।
       TK ঠিক ওই পথেই বন্ধ করেছিলেন ("বিগত দিনের চেম্বার বন্ধ করুন")।
       ⇒ এখন **যে দিনের বোর্ড, সেই দিনের লেখা** — আজ হোক বা পুরনো দিন, নিয়ম এক।
       ⛔ এই একই ফাংশন ছাপা কাগজও ব্যবহার করে, তাই কাগজেও আর অন্য দিনের লেখা
          আজকের নোট সেজে ছাপা হবে না ("⚠️ PROGRESS PENDING" বসবে)।
       ⛔ ওই দিনে সত্যিই লেখা থাকলে কিছুই বদলায় না — টাকার সারির `progress`
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                lifecycleScope.launch {
                    ...
                    val followOk = withContext(Dispatchers.IO) {
                        FollowUpRepository(this@ChamberAttendanceActivity).updateRemark(
                            savedId, text, user.name.ifBlank { user.mobile }, source = "treat", eventDate = selectedDate
                        )
                    }
                    if (!followOk) {
                    ...
                    val reportOk = syncProgressToReportCard(row, text, selectedDate)
                    if (!reportOk) {
                        android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Report Card sync failed — retry", android.widget.Toast.LENGTH_LONG).show()
                        return@launch
                    }
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Treatment saved ✅", android.widget.Toast.LENGTH_SHORT).show()
                    loadBoard()
                }
            }
            .setNegativeButton("Cancel", null)
     ...
     * ⛔ V1653 থেকে এটা আর fire-and-forget নয় — Follow-up + Report Card দুটোই
     *    cloud-এ সত্যি লেখা প্রমাণিত হলেই Treatment saved দেখায়।
     */
    /** V1653 — Treatment is successful only when Report Card progress is really in cloud. */
    private suspend fun syncProgressToReportCard(row: ChamberAttendanceRow, text: String, dateKey: String): Boolean =
        withContext(Dispatchers.IO) {
            try {
                val digits = row.mobile.filter { it.isDigit() }.takeLast(10)
                if (digits.length != 10 || text.isBlank()) return@withContext false
                val dayKey = dateKey.ifBlank { FollowUpModel.today() }
                val todaysPayments = SupabaseClient.fetchList("payments", "mobile=like.*$digits&date=eq.$dayKey", 50)
                val targets = ArrayList<String>()
                val mineRowId = row.patientRowId.trim()
                val mineCode = row.patientId.trim()
                val identityToken = mineRowId.ifBlank { mineCode }
                for (i in 0 until todaysPayments.length()) {
                    val pay = todaysPayments.optJSONObject(i) ?: continue
                    val payType = pay.optString("payType", "")
                    if (payType.equals("bill_edit", true) || payType.equals("chamber_expected", true) || payType.equals("refund", true)) continue
                    val owner = pay.optString("patientId", "").trim()
                    val keep = when {
                        mineRowId.isNotEmpty() && owner == mineRowId -> true
                        mineCode.isNotEmpty() && owner.equals(mineCode, ignoreCase = true) -> true
                        identityToken.isNotEmpty() -> false
                        else -> !PatientModel.isDeclaredSeparateRowId(owner, digits)
                    }
                    if (!keep) continue
                    val pid = pay.optString("id", "")
                    if (pid.isNotBlank()) targets.add(pid)
                }

                if (targets.isEmpty()) {
                    val holder = PaymentModel.buildProgressHolderRow(
                        mobile = row.mobile, name = row.name, branch = row.branch,
                        patientRowId = identityToken, dateKey = dayKey,
                        progress = text, staffMobile = user.mobile
                    )
                    val id = holder.optString("id", "")
                    val ok = try { SupabaseClient.upsert("payments", holder) } catch (_: Throwable) { false }
                    if (!ok || id.isBlank()) return@withContext false
                    val back = SupabaseClient.fetchList("payments", "id=eq.$id", 1, select = "id,progress,patientId,date")
                    return@withContext back.length() > 0 && back.getJSONObject(0).s("progress") == text
                }

                var allOk = true
                for (pid in targets.distinct()) {
                    val fields = org.json.JSONObject()
                        .put("progress", text)
                        .put("updatedAt", SupabaseClient.localWriteStamp())
                    val ok = try { SupabaseClient.updateById("payments", pid, fields) } catch (_: Throwable) { false }
                    if (!ok) { allOk = false; break }
                }
                if (!allOk) return@withContext false
                // Read-back proof: EVERY owned target we updated must carry the exact Progress.
                // This avoids a false success if one network write reports OK but did not persist.
                val wanted = targets.distinct().toSet()
                val confirmed = HashSet<String>()
                val back = SupabaseClient.fetchList("payments", "mobile=like.*$digits&date=eq.$dayKey", 50,
                    select = "id,patientId,payType,progress,date")
                for (i in 0 until back.length()) {
                    val pay = back.optJSONObject(i) ?: continue
                    val id = pay.optString("id", "")
                    if (id in wanted && pay.s("progress") == text) confirmed.add(id)
                }
                confirmed.size == wanted.size
            } catch (_: Throwable) { false }
        }
    ...
    private fun showRemarkDialog(row: ChamberAttendanceRow) {
        ...
        if (row.followUpId.isBlank()) {
            ...
            lifecycleScope.launch {
                // V1653: use the ONE identity-safe resolver/healer. For a shared-mobile
                // secondary patient this can only reuse/create that patient's own row;
                // it never borrows another family member's Follow-up row.
                val resolvedId = try { resolveOrHealFollowUpId(row, digits) } catch (_: Throwable) { "" }
                if (resolvedId == terminalFollowUpSentinel) {
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Entry is Rejected/Incomplete — Restore first", android.widget.Toast.LENGTH_SHORT).show()
                    return@launch
                }
                if (resolvedId.isBlank()) {
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Could not check/create Follow-up record — check connection and try again", android.widget.Toast.LENGTH_SHORT).show()
                    return@launch
                }
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                lifecycleScope.launch {
                    val followOk = withContext(Dispatchers.IO) {
                        try { FollowUpRepository(this@ChamberAttendanceActivity).updateRemark(
                            row.followUpId, remark, staffName, source = "treat", eventDate = selectedDate
                        ) } catch (_: Throwable) { false }
                    }
                    if (!followOk) {
                        android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Failed — retry", android.widget.Toast.LENGTH_SHORT).show()
                    ...
                    val reportOk = syncProgressToReportCard(row, remark, selectedDate)
                    if (!reportOk) {
                        android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Report Card sync failed — retry", android.widget.Toast.LENGTH_LONG).show()
                        return@launch
                    }
                    ...
                    if (board0 != null) {
                        ...
                        val updatedRows = board0.rows.map {
                            if (it.mobile == row.mobile) it.copy(remark = remark, remarkUpdatedAt = nowIso) else it
                        }
            ...
            lifecycleScope.launch {
                ...
                for (row in board.rows) {
                    ...
                    val resolvedId = if (digits.length == 10)
                        try { resolveBestFollowUpId(digits, row) } catch (_: Throwable) { null }
                    else null
    ...
    private fun editRemarkInReview(r: ChamberAttendanceRow) {
        ...
        if (r.followUpId.isBlank()) {
            ...
            lifecycleScope.launch {
                // V1653: same identity-safe resolver/healer as the main Treatment box.
                // A shared mobile can never make Review borrow somebody else's Follow-up.
                val resolvedId = try { resolveOrHealFollowUpId(r, digits) } catch (_: Throwable) { "" }
                if (resolvedId == terminalFollowUpSentinel) {
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Entry is Rejected/Incomplete — Restore first", android.widget.Toast.LENGTH_SHORT).show()
                } else if (resolvedId.isBlank()) {
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Could not check/create Follow-up record — check connection and try again", android.widget.Toast.LENGTH_SHORT).show()
                } else {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                lifecycleScope.launch {
                    val followOk = withContext(Dispatchers.IO) {
                        try { FollowUpRepository(this@ChamberAttendanceActivity).updateRemark(
                            r.followUpId, remark, staffName, source = "treat", eventDate = selectedDate
                        ) } catch (_: Throwable) { false }
                    }
                    if (!followOk) {
                        android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Failed — retry", android.widget.Toast.LENGTH_SHORT).show()
                    ...
                    val reportOk = syncProgressToReportCard(r, remark, selectedDate)
                    if (!reportOk) {
                        android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Report Card sync failed — retry", android.widget.Toast.LENGTH_LONG).show()
                        return@launch
                    }
                    ...
                    android.widget.Toast.makeText(this@ChamberAttendanceActivity, "Treatment/Remark updated", android.widget.Toast.LENGTH_SHORT).show()
                    // V1653: Follow-up + Report Card were both cloud-confirmed above before success.
                }
            }
            .setNegativeButton("Cancel", null)
            .show().also { PremiumAlert.paint(it) }
    ...
    // 🔴🔒 V1619 (২০.০৯.২০২৬, TK-নির্দেশ, ডেমো-প্রুফ পাশ) — Search Patient-এর
    // ফলাফলের বাক্স আগে সবসময় স্থির 400dp-তেই আটকে থাকত, ফোনের আসল উচ্চতা
    // যা-ই হোক। এখন ডাকার জায়গা চাইলে নিজের উচ্চতা দিতে পারে; না দিলে আগের
    // 400dp-ই থাকে — তাই বাকি সব ডায়ালগ (Mark Expected ইত্যাদি) এক অক্ষরও বদলায়নি।
    private fun premiumDialogShellChamber(titleText: String, bodyMaxHeightDp: Int = 400): ChamberDialogParts {
        val d = resources.displayMetrics.density
        fun dp(v: Int) = (v * d).toInt()
        ...
        val scroll = android.widget.ScrollView(this).apply {
            addView(body)
            layoutParams = android.widget.LinearLayout.LayoutParams(android.widget.LinearLayout.LayoutParams.MATCH_PARENT, dp(bodyMaxHeightDp))
        }
        root.addView(scroll)
