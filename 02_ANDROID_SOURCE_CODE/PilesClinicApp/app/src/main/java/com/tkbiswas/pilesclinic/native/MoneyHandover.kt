...
object MoneyHandover {
    ...
    fun verifyPassword(mobile: String, role: String, typed: String): Verify {
        ...
        when (CloudPasswordCheck.verifyPasswordV2(digits, typed)) {
            CloudPasswordCheck.VerifyV2.OK -> return Verify.OK
            CloudPasswordCheck.VerifyV2.WRONG -> return Verify.WRONG
            CloudPasswordCheck.VerifyV2.NO_CUSTOM ->
                return if (typed == StaffDirectory.defaultPasswordFor(role)) Verify.OK else Verify.WRONG
            CloudPasswordCheck.VerifyV2.ERROR -> { /* proven old path below */ }
        }
